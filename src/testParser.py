...
class TestParser(object):
    ...
    def parse(self):
        ...
        test = {'__raw_lines__': raw_lines, 'path': self.path, '__emit__': []}
        ...
        while (i < len(lines)):
            ...
            if re.match('\A\s*\Z', lines[i]):
                ...
                i += 1
                continue
            if m := re.match('\A([^"]*?):\s*"([^"]*)"\s*\Z', lines[i]):
                test[m[1]] = m[2]
                test['__emit__'].append(("oneline", m[1]))
                i += 1
                continue
            if m := re.match('\A([^"]*?):\s*"""\s*\Z', lines[i]):
                msg = []
                ...
                while(not re.match('\A\s*"""\s*\Z', lines[i])):
                    ...
                    i += 1
                test[m[1]] = '\n'.join(msg)
                test['__emit__'].append(("multiline", m[1]))
                i += 1
                continue
